...
- **Expanded Report** — off or on. Off, the records live behind a select
  ...
  chips recount to that record. The same layout on the phone and the desk; they
...
- **Liquid Glass navigation** (mobile). One material, two navigations. The
  ...
  icon on that surface rather than a disc on it. The grey runs unbroken from
  the top of the screen down through the title, because the hour-and-battery
  strip paints `--headBg` — the header's own ground, declared once where the
  phone declares it — instead of a colour of its own, which is how that strip
  ended up a shade darker than the header beneath it.
