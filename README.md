...
9. **Mobile** — bento layout with the mini organ carousel, metrics carousel and the signal insight sheet. The organ card used to open an organ-age sheet on tap; a tap now does nothing, and the card is a thing to swipe and to look at. Above the biomarker list, the **filter chips** are controls: fully rounded, a filled plate with no line, 44px tall on white at 8%, the chosen one a step lighter at 16% behind a 1.5px white ring, and the label at full strength in both themes — white on the dark ground, the theme's own ink on paper — an inset ring rather than a border, since Chromium floors border-width to whole CSS pixels and a declared 1.5px border paints at 1 — and being outside layout it cannot shift the row at all. The counts are tallied from the rows: All is the sum of the rest, which is also the row count, and a range holding nothing keeps its chip, at zero: five chips whatever is on the page, so the row does not reshuffle as the reader narrows it, and a chosen range that empties under a narrower record stays chosen — the empty line says why, which is more use than the page quietly changing the filter out from under the reader. One at a time, and a tap runs the list through the same loading beat a keystroke does: the skeleton stands in for 380ms, then the filtered set arrives. The chips and the search are one filter rather than two, so choosing *Optimal* inside a search narrows what the search found. Where a range holds nothing the empty line says so in the product's words — *No biomarkers out of range* is a result, not a list that failed to load. Below them the panel runs to seven groups and **110 readings** — 68 optimal, 20 suboptimal, 12 out of range, 10 unclassified, which is the design's own figure and adds up to it. The rows are built from a table in the script rather than written as markup: the counts are then a property of the data, so a chip's number is the rows it will show by construction. Each row's marker is placed from its status, so an optimal reading sits over the green and an out-of-range one past the amber. **Above** the chips — the select and the search decide which set is on the page, the chips cut that set by range — the **record select and the search share a row**, both 56px on a 14px radius. At rest the search is a 56px square with the magnifier in it and nothing else — a field that says *Search* and holds no text was spending 40% of the row on a word — and the select takes everything left over, 81%, which is room for the report's title over its date. Tapped, the field opens to 50% and the select gives the room back at 50% — and it opens **leftward**, the right edge holding still, which needs both widths to be lengths rather than `auto`, since flex-basis interpolates between two lengths and jumps between `auto` and one, dropping its eyebrow while it is narrow: category and date are the detail you drop first, and one line that can be read beats two that cannot. Both are flex-basis with a transition, so the swap is two boxes changing width rather than a reflow, and the collapsed field is zero-wide rather than `display:none`, since the tap opens the search *by focusing it* and a `display:none` element cannot take focus. It stays open while it holds text — a search that collapsed to an icon would be a filter you cannot see or undo. Collapsed, the magnifier is 20px and centred (the box drops its 9px gap, or the icon centres against a field that is not there and lands 4.5px left). Both shells carry a **1.5px inset ring** rather than a border — Chromium floors border-width to whole pixels, so a declared 1.5px border paints at 1 — coloured through one `--msEdge` token, which goes full strength on focus: white on the dark ground, the theme's own ink on paper, the same edge the chosen chip wears. The select carries the report's title at 13px over its category and date when one is chosen, one line for *All records* — the two lines are close in size and the colour carries the hierarchy: the select names which set of results is on the page and opens a menu anchored under it — each record a title, its *category | date*, and a badge with the readings it carried. The badge counts are the design's and sum to 174 over a panel of 110, which is right: a biomarker measured twice is in both reports, so a reading belongs to one or more records and *All records* is the 110 distinct. Every badge is exactly what its option shows, and the chips recount for whichever record is chosen. Picking one runs the list through the same loading beat a keystroke and a chip do. The whole filter group is sticky — the select, the search **and** the chips as one bar: it pins under the status bar (the hour, the battery) on the phone's own ground as the list scrolls — and the strip above it belongs to the bar: one surface from the top of the screen through the fields and the chips, ending in the fade, with the hour and the battery floating over it. The bar's plate reaches up by the status bar's height plus whatever is still to run before it pins, plus a pixel of overlap into its own background; the status bar needs no plate of its own: it is back in the flow, so the page starts below it and nothing passes beneath the clock. Neither plate fades: a plate with animated opacity is a gradient over whatever is behind it, which is what a fixed bar must not have. The organ no longer runs under the hour at rest, which costs nothing — it starts well below the strip anyway — and buys a top bar with no state in it. Where both apply they cover the same strip twice rather than meeting along an edge — two surfaces that have to meet perfectly will eventually fail to, so it is always there to tap, and where it pins the plate carries on past its own bottom as a 30px fade of the page's ground — a shadow marks that edge, it does not soften it, so rows go out under the bar rather than being sliced by it. The chips were left in the scrolling list when the select and the search moved above them, which read as a bug and was one: once the bar pinned they slid up behind it and stood there cut across their own middle, a row of half pills under the plate that looked like a second bar. They are the same filter as the select, so they pin with it and the list passes under one edge. Inside the bar they keep the full width — negative margins hand back the bar's own side padding, since a horizontal scroller that stops short of the edge says it has ended when it has not. Focused, the page scrolls up to it and the main navigation leaves while the keyboard is up — on the device the real one, in the frame a drawn iOS keyboard that slides up from the bottom edge, and back down when the field lets go, whose keys type into the field; typing shows a short skeleton (the list's own rows, every text line and pill a shimmering bar, so the swap to results moves nothing) and then filters the groups by name, with an empty line for a miss; the cross clears the text and nothing else — no scroll, no change of focus; a scroll by the reader lets the field go, so the keyboard drops, and a tap brings it back. The results area stays screen-tall only while the field is focused, so the page is never longer than its content once the keyboard is gone. The tab bar keeps 18px plus the safe area above the home indicator, and in the frame draws the indicator itself, white at 26% (black at 24% in light)
...
15. **Tablet** — Desktop V5 in a frame two hundred pixels narrower, and a page rather than a state of one. It was a tweak on V5 first, which is what a shell looks like before you try to link to it: a design you cannot open on its own, cannot put beside its siblings in the menu, and cannot point anyone at. A page costs one index and one class chain — it wears every rule V5 wears plus the `tabM` marker the layout is written against — and buys all three back. One thing has to give at that width: the organ card cannot hold the list and the organ side by side, and halving the organ to keep ten rows would be keeping the least of the card. So the list goes and the card reads the way the phone's hero does — the organ centred with the whole width, arrows either side of the number, and the names on the floor of the card with their neighbours showing, cropped by the card's edge rather than shortened. The number, its arrows and the caption sit 16px higher than Desktop V5 puts them, the name strip 16px further off the card's floor, and the names are medium weight, the active one included. Nothing is lost: every reading the list held is one arrow away, and the one the card is showing is the one it was already showing large. The strip and the arrows count by the same sequence the phone's carousel does, so the organ ages collapse to a single *Organ age* slide when they are unread, exactly as they do there. Everything else is the same page tightened — a narrower rail, a shorter page margin, the left column stacked one-up because half of a narrower page is not two cards wide, one row of filter chips that scrolls rather than wraps — the chips, the record select and the search wear the phone's own shells at every width now, though not in the same arrangement: the desk and the tablet put the record select and the search at either end of one full-width row, with nothing between them — they are the two ends of one question, which set is on the page and what in it you are looking for — and give the chips the second row to themselves, since the chips are the one of the three that grows. Both rows pin to the top of the page as one block, as the phone's bar does — the controls that decide what the list shows should still be there a thousand rows in — with the page's ground behind them and the same 30px fade underneath, on only once it is actually pinned. That ground is a token (`--pageBg`, with `--pageBg0` its zero-alpha twin for the fade) declared wherever each theme declares its page colour: the desk is `#0d0d0c`, the tablet and b5 are `#000`, light is white. Naming the colour a second time is what made the block a grey slab on the tablet. Both fields are 256px. The chips scroll rather than wrap and their last 28px fade, so a chip cut at the edge reads as more to scroll rather than as broken. The phone stacks all three, because 375px cannot hold any of it side by side. 56px on a 14px radius for the two fields, 44px fully rounded on a filled plate for the chips: one component, one set of numbers and one arrangement rather than a desk version and a phone version of the same control — and one behaviour: the desk's select opens the same anchored record menu, its search is a real field with a clear cross, its chips are buttons that count the rows they will show, and all three run the list through the same 380ms loading beat. The desk's list is built from the same table the phone's is, so the same record narrows both surfaces to the same 110 readings; it used to be six placeholder rows under chips carrying the design's figures, which is two sources for one number. The filter itself lives in one place (`bioFilter`) that both surfaces call with their own elements — the phone additionally holds the scroller's height through the beat, since a page that shortens under a pinned bar unpins it, and lands at the top of the results, since a scroll position measured in the old list means nothing in the new one; the desk has neither a pinned bar nor a keyboard to work around. The pair's row is 560px rather than the page's full width — the ratio is the phone's, but 60% of a 1160px column is a select with half a foot of empty space inside it, which is a different control, not the same one bigger. Where the tablet drops the search the row ends where the select does, and the select keeps the width it has at the desk: a control that grows when its neighbour leaves is a different one, and a table that drops the sparkline, the one column that says least at a glance. Coverage and steps stay side by side as they are on the desktop — stacking them was the obvious move and the wrong one, since the pair is one row of two readings taken together rather than two cards that happen to be adjacent.
16. **Teaser video** — the storyboard for the thirty-second teaser, a page of its own under *Video* (`#m22`). One frame in focus on the left — a 16:9 stage with the spoken line on it as a subtitle, bottom centre in white, and the motion notes under it, the After Effects pass needs: what moves, the easing, the durations, the cut — and the script with timecodes on the right — in four acts, Introduction, New feature, Value and Closure, each line choosing the frame it belongs to (the arrows in the frame's bar and the arrow keys step through; the board plays itself from the start and plays the voice with it: each sequence sets the recording's head to its own line, plays it, then holds a beat before handing on, so the cuts fall in the pauses; the captions are written at the voice's pace, every sketch's own motion is scaled to fit its slot, and the music runs under the whole thing, ducking while the voice speaks and opening back up in the pauses — *Pause*, or the space bar, holds it, and a click on a row joins the play there — and a hairline inside each act of the script, under its heading, fills as the sequence in focus plays, through its row on the board's clock, whether the play is on or a row was clicked); no title over it, the frame is the page. The page takes the whole viewport, 64px in, on a #111 ground, and everything on it sits in black cards with a 32px radius and 32px of padding: the frame, the script, and the palette, voice, type and delivery notes — the sound card carries the take the board plays, the music bed under it and the style reference the read was matched to. Nine sequences, each a loose animated sketch in the product's own system — the three inks, the arc, the halo, the type. The type inside a stage is sized to the stage, so a frame reads the same at any width. It opens on **one dot**: black, a single dot at the centre, close, and a zoom out that shows it to be one of a universe of dots, the readings typing on through the rest of the pull-back — one sequence, one shot — the field drifting consistently in one direction like a pull, in four depths at four speeds so it parallaxes, the far ones blurred, the nearest out of focus, the middle sharp — while readings surface in the sharp depths and go again through the zoom out. Frame 02 is the Library's body, exactly — the same view with the same options the Library tile is built with — reached by the concept's morph: it opens on a loose cloud of the body's own particles, and they travel, point for point, into the body while the camera pulls back a touch; as the camera settles two chips are written on over the body's shoulders, small mono in black, one after the other: *124 biomarkers* on white to the left, then, once it has finished, *8 years younger* in the accent to the right, and the body holds while the second line lands. The sequences: 01 one dot becoming the universe of signals, the readings typing on through the zoom out; 02 the cloud of particles travelling into the body, with the chips — *together, they tell a bigger story*, then, its own subtitle, *but not the full story*; 03 *Introducing*, then *Organ ages*, each turning in on a vertical axis on black; 04 the app photographed on a phone, close on the heart, a slow push in — *a deeper view of your organs*; 05 the same app on a tablet, every organ age listed — *some age exactly as they should*; 06 the phone at an angle, the heart in the orange, 44 — *others are ageing faster*; 07 every organ age at once, on a tablet turned away from the camera, the cards fading up one after the other as it drifts back; 08 *Available now*, then *Look closer*, as 03, then a cut; 09 the wordmark, the dot, black. Palette, type and delivery notes close the page. The page is black in every theme, since a video is graded on black. Built on first view; the organs stop painting when the page is hidden.
...
17. **Organ Age (landing page)** — the teaser opened out into a page you scroll (`#m23`). One stage pinned to the viewport, one particle system, ten beats on a single timeline, and no two consecutive keys sharing a camera and a pan — a hold holds the morph, not the camera, so through every one of them the camera keeps pushing in and the world keeps rising, and scrolling never produces a still:  a dot pulls back into a body, the body opens into a universe of signals, the universe goes out for a title card, the particles come back as a heart, the heart's reading arrives, it comes apart and reorganises as a lung, the camera pulls back to five organs at once, and they resolve into the phone. Scroll position is the only clock — every scene is the same points at different targets, blended, so a particle in the universe is the one that becomes a chamber of the heart and the one that lands in the phone. Nothing is a second copy of the design: the organs are the file's own clouds through `samplePts`, the colours its three inks through `inkNow`, the opacities its twenty-step grid through `opRender`, the dots batched by `makeBatch`, and the figures the product's own (`PILL`, `CHRONO`). What is new is a camera — a perspective divide and a defocus term, both driven from the timeline. Black in every theme. Built on first view. **The second organ is the lung, not the kidney**: the brief suggested kidneys, and in the product's own table the kidney reads a year *younger*, so using it for "ageing faster" would have the page contradict every other screen in the file.

18. **Library** — behind a left nav: **Organ Library** (every organ isolated on its own tile, the master component each surface mounts), **Organs — Fine Grain** (the same organs at twice the particles) and **Colour** (the three ink tokens, each over the twenty steps of the opacity scale — 5% to 100% in fives — read live from the same values the engine paints from, so setting a token on the tweak bar moves the ramp with the cloud, and the theme swaps all three; click a step to copy its `rgba()`). Every tile reports its dot count and downloads as PNG, JPG or SVG.
...
- **Unclassified** — off by default. The biomarker panel has two readings of
  ...
  *10 Unclassified*, with the meter's grey remainder for its dot and the meter
  ...
  theme. Dark is the design's tokens as given: optimal / younger `#AAEEC2`,
  suboptimal / older `#FFE29F`, out of range `#FE8087`, aligned / unclassified
  white at 48%. Light keeps the set read off the mocks (`#6fcb7c`, `#f2c550`,
  `#e5514f`, `#7f7f7f`). They are CSS variables that every legend dot, meter
  run, range bar and the hero's distribution arc read; the unclassified colour
  is also the meter's remainder. On the dark ground the age arc takes the same
  tokens — younger in the optimal colour, older in the suboptimal running to
  out-of-range by seven years, the aligned centre tick white at 48% — with no
  desaturation, and the younger / older dots in the lists and pills follow. Every V6 hero figure,
...
- **Bio graph** — Arc (default) or Dots, in the Tweaks dropdown; a select, since
  ...
  little over a dot wide (4.5px to a 3.7px dot) — filled left to right with
  the ranges' colours by share
...
- **Expanded Report** — off or on. Off, the records live behind a select
  ...
  table the menu reads, and both controls call one `choose()` in `bioFilter`,
  so neither can say one thing while the list shows another. Picking a card
  runs the list through the same 380ms beat a chip and a keystroke run, and the
  chips recount to that record. Through all of it the pinned filter block does
  not move: the skeleton is held at the height the list had (or a screenful
  under the block, whichever is more) and the results keep that screenful
  afterwards, so nothing shortens, the browser never clamps the scroll, and the
  bar is where it was left when the new list arrives. The same layout on the phone and the desk; they
  differ only in how much room the row has, which is what scrolling is for. Not
  on the first desktop page, which filters by chips alone and has no record
  select or search for the tweak to rearrange.
- **Left card** — the latest report (the default), or Coverage. A select rather than a toggle, unlike the rows around it: Coverage is not the *off* state of Latest report, it is the other thing that could be there, so `stLeft` is one of the few ids `segmentise` leaves as a select. Coverage is a reading the page
  ...
  `.hiCov` — which the report card wears so the slot's rules land on it — left
  the real Coverage card behind and emptied the slot when the choice flipped. Title, date and count
...
**Colour is random. Opacity is not.** That split is the whole treatment. (The bar's Ink swatches each carry a hex field beside them: the browser's picker only offers RGB, so the hex is typed or read there, and swatch and field move together.)
...
- **Liquid Glass navigation** (mobile). One material, two navigations. The
  ...
  the title is a header rather than a pane: it sits inside `.mHead`, which
  already carries the hero's grey and is the thing that sticks, so a pill on
  top of it was a plate on a plate. Flat, full-bleed, with the share button an
  icon on that surface rather than a disc on it. The header begins **below**
  the hour and the battery: that strip is in the flow, on the phone's own
  ...
  inactive colours. The assistant's pill beside them is the island's material a step brighter, not a colour: it carried the accent as a fixed tint for a while, which is the one thing in the dock that contradicts the material's premise — every other pane takes its colour from what is behind it, and on a black page nothing behind that one is orange, so what showed was the raw tint reading as brown. It carries a sparkle rather than a speech
  bubble -- a bubble opens a thread, a sparkle opens something that answers --
  drawn solid like the five and at 20px, since it fills five sixths of its box
  where the stroked bubble filled two thirds.
  ...
  **The dock is tunable from the bar** — one *Dock → Style* dropdown, shown on
  ...
  halves: **Shape**, then **Material**. It was two menus for a while, on the
  reasoning that *how much room the dock takes* and *what the glass is like*
  are different questions. In use they are not — tuning a dock means moving
  between them, and two buttons that close each other made that a chore, so
  everything about the dock is in the dock's menu. Each control writes one
  custom property on the root element; the CSS is the material, so there is no
  second copy of the numbers and nothing re-renders.
  ...
  **And the dock's shape, above it in the same panel** — Shape (island + chat, or one bar), Labels (shown, or icons only),
  ...
  the device draws its own. The height drives the island, the chat, both radii **and** the
  ...
  a design system rather than eyeballed off a thumb. One **Save** and one *Reset* at the foot of the panel serve both halves: two stored tables, but a designer tuning a dock is tuning one object and saves it once. The setup survives a reload, as the ink pickers always have. *Reset* removes the properties rather than writing the defaults back — and clears the saves too, or a reset would look undone the moment the page reloaded.
