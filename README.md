...
16. **Teaser video** — the storyboard for the thirty-second teaser, a page of its own under *Video* (`#m22`). Nine sequences in a three-by-three grid, each frame a loose animated sketch in the product's own system — the three inks, the arc, the halo, the type — with the spoken line under it and the motion notes the After Effects pass needs: what moves, the easing, the durations, the cut. The script sits beside the title with timecodes. Frames 04–06 draw the real component: the particle heart and lungs are the painter's, the same tiles the Library draws, on a drawn phone with the arc revealing to the reading and the halo in the verdict's light; frame 06 loops the heart-to-lungs morph so both states can be seen. The sequences: 01 a field of readings drifting; 02 the field drawing in to one accent point; 03 *Introducing Organ Age* breathing on black; 04 the heart, green arc, 37; 05 a slow push in on the same; 06 the lungs, orange arc, 48; 07 heart age over time with what to do next (the board's line, not in the VO); 08 *Look closer.* then a cut; 09 the wordmark, the dot, black. Palette, type and delivery notes close the page. The page is black in every theme, since a video is graded on black. Built on first view; the organs stop painting when the page is hidden.
