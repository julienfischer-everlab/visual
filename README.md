...
- **Biomarkers**, **Activity**, **Biological age**, **Organ age** — each can be
  ...
  **Biomarkers** is the age card's state in the hero: on V6 the slide stays in
  ...
  Emptied outright, an organ slide stops saying nothing with a dash and says
  what would fill it instead: *Unlock your organ ages* and *Get tested*, in the
  slot the number had, with the silhouette above it still grey and the arc
  still stood down. It is deliberately shorter than the biomarker slide's —
  that one is alone in its carousel and can take a paragraph, while these nine
  still slide past each other inside the age strip's 110px box, and anything
  taller is clipped by the strip's own edge fade.

  *Partial* is organ age's middle case: some organs have been read and some
  have not, and it keeps the dash — the reading exists for most of them, and an
  action on every unread one would be the same sentence nine times over a
  carousel that is working.
