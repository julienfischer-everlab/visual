...
16. **Teaser video** — the storyboard for the thirty-second teaser, a page of its own under *Video* (`#m22`). One frame in focus on the left — a 16:9 stage, the spoken line under it and the motion notes the After Effects pass needs: what moves, the easing, the durations, the cut — and the script with timecodes on the right, each line choosing the frame it belongs to (the arrows in the frame's bar and the arrow keys step through); no title over it, the frame is the page. Ten sequences, each a loose animated sketch in the product's own system — the three inks, the arc, the halo, the type; two have no voice-over and are marked. The type inside a stage is sized to the stage, so a frame reads the same at any width. It opens on **lifestyle imagery**: a warm still (someone reading on a sofa; the sketch stands in for the photograph with its tones) that pushes in and then breaks into a dot screen carrying the image's own colours, through which the field of signals comes up — the transition from the world to the cloud of dots. Frames 04, 06 and 07 draw the real component: the particle heart and lungs are the painter's, the same tiles the Library draws, on a drawn phone with the arc revealing to the reading and the halo in the verdict's light; frame 07 loops the lungs-to-heart morph so both states can be seen. Frame 08 plots three organs over time as three lines, each drawing on in turn in its verdict's colour and never merging — *every organ tracked independently, for better insights and better decisions* — with what to do next under them. Frame 03 is the product's own body and a zoom out: it opens close on a few particles drifting at the edge of a cloud (the engine's sphere), and the camera pulls back as the cloud becomes the particle body, particle for particle, by the component's own morph; as the camera lands a chip pinned to the body's shoulder reads *8 years younger* in the accent. The sequences: 01 the still; 02 the field of readings; 03 the cloud becoming the body, with the chip; 04 the heart, green arc, 37 — *your organs age differently*; 05 *Introducing Organ Age* breathing on black; 06 a slow push in on the heart — 07 the lungs, orange arc, 48; 08 over time; 09 *Look closer.* then a cut; 10 the wordmark, the dot, black. Palette, type and delivery notes close the page. The page is black in every theme, since a video is graded on black. Built on first view; the organs stop painting when the page is hidden.
