...
- **Arc cycle** — mobile only, off by default, in the Tweaks dropdown when a
  phone page with the V6 hero is up. On, the biomarker arc's band keeps its
  extent and its indicator, and its one colour walks the four statuses —
  optimal, suboptimal, out of range, unclassified — two seconds each, with a
  350ms crossfade into each, looping without a pause. Never more than one
  colour on the arc at a time, whichever way the Unclassified switch is set;
  the inactive ticks stay their low grey and nothing else on the slide changes.
