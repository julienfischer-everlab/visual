...
16. **Teaser video** — the storyboard for the thirty-second teaser, a page of its own under *Video* (`#m22`). One frame in focus on the left — a 16:9 stage with the spoken line on it as a subtitle, bottom centre in white, and the motion notes under it, the After Effects pass needs: what moves, the easing, the durations, the cut — and the script with timecodes on the right, each line choosing the frame it belongs to (the arrows in the frame's bar and the arrow keys step through); no title over it, the frame is the page. The page takes the whole viewport, 64px in, on a #111 ground, and everything on it sits in black cards with a 32px radius and 32px of padding: the frame, the script, and the palette, type and delivery notes. Ten sequences, each a loose animated sketch in the product's own system — the three inks, the arc, the halo, the type; two have no voice-over and are marked. The type inside a stage is sized to the stage, so a frame reads the same at any width. It opens on **one dot**: black, a single accent dot at the centre, close, and a zoom out that shows it to be one of a universe of dots — the same field 02 continues, drifting consistently in one direction like a pull, in four depths at four speeds so it parallaxes, the far ones blurred, the nearest out of focus, the middle sharp — while readings surface in the sharp depths and go again through the zoom out. Frame 03 is the Library's body, exactly — the same view with the same options the Library tile is built with — and a zoom out: it opens close on the particles and the camera pulls back until they resolve into the body; as the camera settles a chip pinned to the body's shoulder reads *8 years younger* in the accent. The sequences: 01 the one dot; 02 the universe of signals; 03 the cloud becoming the body, with the chip; 04 the heart, green arc, 37 — *your organs age differently*; 05 *Introducing Organ Age* breathing on black; 06 a slow push in on the heart — 07 the lungs, orange arc, 48; 08 over time; 09 *Look closer.* then a cut; 10 the wordmark, the dot, black. Palette, type and delivery notes close the page. The page is black in every theme, since a video is graded on black. Built on first view; the organs stop painting when the page is hidden.
